...
public class DialogsAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            holder.dialogImageView = convertView.findViewById(R.id.image_opponent_icon);
            ...
            holder.dialogAvatarTitle = convertView.findViewById(R.id.tvShortName);
        ...
        if (dialog.getName()!= null && !dialog.getName().trim().isEmpty()) {
            if (dialog.getName().trim().contains(" ")) {
                String[] eventName = dialog.getName().trim().split(" ");
                holder.dialogAvatarTitle.setText(eventName[0].substring(0, 1).toUpperCase() + eventName[1].substring(0, 1).toUpperCase());
            } else if (dialog.getName().trim().length() > 2) {
                holder.dialogAvatarTitle.setText(dialog.getName().trim().substring(0, 2).toUpperCase());
            } else {
                holder.dialogAvatarTitle.setText(dialog.getName().trim().toUpperCase());
            }
            holder.dialogAvatarTitle.setBackgroundDrawable(UiUtils.getColorCircleDrawable(dialog.getUserId()));
        }
